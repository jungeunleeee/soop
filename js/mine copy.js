...
function preventDefaultAnchor() {
  ...
  function mainSlideShow(selector) {
    ...
    $selector.find('li img').on('load', function () {
      loadCounter++;
      if (loadCounter === $('#main-visual .banner > li').length) {
        if (isTimerOn === true) {
          stopTimer();
          slideShow(slideFirst);
        }
    ...
    // hover 했을 때 이미지 보여주기
    $('#main-visual ul.actor-list li a').mouseenter(function () {
      stopTimer();
      var index = $('#main-visual ul.actor-list li').index($(this).parent());
      slideShow(index + 1);
      showSlide(index + 1);
    });

    $('#main-visual ul.actor-list > li > a').on('focus', function () {
      var index = $('#main-visual ul.actor-list > li').index($(this).parent());
      slideShow(index + 1);
    });

    // 마우스가 떠났을 때 다시 시간 작동하기
    $('#main-visual ul.actor-list li a').mouseleave(function () {
      resetTimer();
      startTimer();
    });

    // 이미지가 다 로드 되었을 때 슬라이드 전환 시작
    function startTimer() {
    ...
    function slideShow(n) {
      ...
      if (slideNow === 0) {
        $selector.find('ul.actor-image > li:eq(' + (n - 1) + ')').addClass('on');
      } else {
        onAnimation = true;
        $selector.find('ul.actor-image > li:eq(' + (slideNow - 1) + ')').removeClass().addClass('hide').one('animationed', function () {
          $(this).removeClass();
          onAnimation = false;
        });
        $selector.find('ul.actor-image > li:eq(' + (n - 1) + ')').addClass('show');
      }
      slideNow = n;
